...
def spatial_pyramid_pool(self,previous_conv, num_sample, previous_conv_size, out_pool_size):
    ...
    for i in range(len(out_pool_size)):
        ...
        h_wid = int(ceil(previous_conv_size[0] / out_pool_size[i]))
        w_wid = int(ceil(previous_conv_size[1] / out_pool_size[i]))
...
class FocalLoss2d(nn.Module):
    ...
    def forward(self, inputs, targets):
        #print 'shape Focalloss ',inputs.shape,targets.shape
    ...
    def wrapper(self, x):
        ...
        if self.training and self.use_aux:
            ...
            for s in self.scales:
                ...
                scaled_x = F.upsample(x, size=new_size, mode='bilinear',align_corners=True)
